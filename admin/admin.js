document.addEventListener("DOMContentLoaded", () => {
  ...
  archivoImagen.addEventListener("change", function() {
    ...
    if (file) {
      urlImagenInput.value = ""; // Limpiar el link si elige archivo
  ...
  urlImagenInput.addEventListener("input", function() {
    if (this.value.trim() !== "") {
      archivoImagen.value = ""; // Limpiar archivo si escribe link
  ...
  btnAgregar.addEventListener("click", async () => {
    ...
    // 2. Segunda opción: Link pegado
    else if (urlImagenInput.value.trim() !== "") {
        urlFinal = urlImagenInput.value.trim();
    }
  ...
  // --- El resto de tus funciones (Login, Categorías, Toast) se mantienen igual ---
  // ... (btnLogin, btnLogout, auth.onAuthStateChanged, cargarCategoriasSelect, btnAgregarCategoria, mostrarToast)
  
  // (Asegúrate de incluir las funciones de Login y mostrarToast que ya tenías)
